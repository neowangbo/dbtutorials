...
return date is
begin
    return to_date(to_char(d_date,'SYYYYMMDD')||'235959','SYYYYMMDDHH24MISS');
end end_of_day;
...
begin
    n_year := round(DBMS_RANDOM.value(n_starting_year,n_ending_year),0);
    --pl('n_year='||n_year);
    loop
...
--      1234567890123456789012345678901234567890123456789012345678901234567890
    ...
    pl('d_MIN');
    pl(chr(9)||'Represents the minium value for the DATE data type.');
    pl(chr(9));
    pl('----------------------------- FUNCTIONS ------------------------------');
    pl(chr(9));
    pl('DATE_.end_of_day(');
    ...
    pl(chr(9));
    pl('DATE_.random(');
    pl('    n_starting_year in number,');
    pl('    n_ending_year   in number)');
    pl('return date;');
    ...
    pl('    d_date in date)');
    pl('return        date;');
    pl(chr(9)||'Returns the passwd date with the time portion set to the start');
    pl(chr(9)||'of the day:');
